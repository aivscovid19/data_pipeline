...
class SiteWorkerIntegrated():
  ...
  Attributes:
    ...
    max_delay (int): A default value of max seconds to wait before the
                      next request to a website is sent.
    article_schema(list): Schema for articles in BigQuery.
  '''
  ...
  def __init__(self, driver_path=None): 
    ...
    self.min_delay = 0.1
    self.max_delay = 2
    self.article_schema = DataTable().schema
  ...
  def scrape_data(self, miner, url, article_schema):
...
class IbmcRuSiteWorker(SiteWorkerIntegrated):
  ...
  def scrape_articles(self):
    ...
    return self.scrape_data(miner, self.url, self.article_schema)
...
class MedrxivSiteWorker(SiteWorkerIntegrated):
  ...
  Attributes:
    ...
    article_schema (list of dicts): An default article_schema for
                                      a given SiteWorker.
  '''
  def __init__(self, url, limit=100, driver_path=None):
    super().__init__()
    self.url = url
    self.driver_path = driver_path
